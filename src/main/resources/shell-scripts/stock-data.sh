...
DB_USERNAME=ENV_DB_HOST
DB_PASSWORD=ENV_DB_PASSWORD
DB_NAME=END_DB_NAME
HISTORICAL_STOCK_PRICES_OUTPUT_FILE=${SCRIPT_DIR}/historical-stock-prices-output.txt

export SPRING_DATASOURCE_URL="jdbc:mysql://localhost:3306/${DB_NAME}?rewriteBatchedStatements=true"
...
if [ -f "${HISTORICAL_STOCK_PRICES_OUTPUT_FILE}" ]; then
    mysql --local-infile=1  -u${DB_USERNAME} -p${DB_PASSWORD} ${DB_NAME} -e "LOAD DATA LOCAL INFILE '${HISTORICAL_STOCK_PRICES_OUTPUT_FILE}' INTO TABLE historical_stock_prices FIELDS TERMINATED BY '|' LINES TERMINATED BY '\n' IGNORE 0 LINES"
